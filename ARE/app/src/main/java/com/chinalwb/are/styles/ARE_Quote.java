...
public class ARE_Quote implements IARE_Style {
	...
	private void makeLineAsQuote() {
		...
		QuoteSpan[] existingQuoteSpans = editable.getSpans(start, end, QuoteSpan.class);
		if (existingQuoteSpans != null && existingQuoteSpans.length > 0) {
			return;
		}
		if (start > 2) {
			existingQuoteSpans = editable.getSpans(start - 2, start, QuoteSpan.class);
			if (existingQuoteSpans != null && existingQuoteSpans.length > 0) {
				// Merge forward
	...
	private void removeQuote() {
		...
		int start = Util.getThisLineStart(editText, currentLine);
		int end = Util.getThisLineEnd(editText, currentLine);
		QuoteSpan[] quoteSpans = null;
		if (start == 0) {
			quoteSpans = editable.getSpans(start, end, QuoteSpan.class);
			editable.removeSpan(quoteSpans[0]);
			return;
		} else {
			quoteSpans = editable.getSpans(start - 1, end, QuoteSpan.class);
		}
	...
	public void applyStyle(Editable editable, int start, int end) {
		...
		if (end > start) {
			...
			// User deletes
			QuoteSpan quoteSpan = quoteSpans[0];
